...
def test_build_quality_report(tmp_path: Path) -> None:
    write_summary(tmp_path / "run-summary-2026-06-17.json", 10, True, duration_seconds=90)
    write_summary(tmp_path / "run-summary-2026-06-16.json", 0, False, ["low items"], duration_seconds=150)
    report = build_quality_report(tmp_path, days=7, fallback_daily=False)
    assert report["runs"] == 2
    assert report["success_rate"] == 0.5
    assert report["average_items"] == 5
    assert report["average_duration_seconds"] == 120
    assert report["min_duration_seconds"] == 90
    assert report["max_duration_seconds"] == 150
